...
export class AutoValidationService {
    ...
    constructor() {
        ...
        ajvErrors(this.#ajv);
        this.#ajv.addFormat('MODELLNUMMER', {
            type: 'string',
            validate: this.#validateMODELLNUMMER,
        });
    }
    ...
    #checkChars(chars: string[]) {
        ...
        if (chars.length === 9) {
            // Compute the MODELLNUMMER-10 check digit
            chars.reverse();
            ...
            // Compute the MODELLNUMMER-13 check digit
            for (let i = 0; i < chars.length; i++) {
    ...
    #validateMODELLNUMMER: FormatValidator<string> = (subject: string) => {
        // Checks for MODELLNUMMER-10 or MODELLNUMMER-13 format
        ...
        const regex =
            /^(?:MODELLNUMMER(?:-1[03])?:? )?(?=[\dX]{10}$|(?=(?:\d+[- ]){3})[- \dX]{13}$|97[89]\d{10}$|(?=(?:\d+[- ]){4})[- \d]{17}$)(?:97[89][- ]?)?\d{1,5}[- ]?\d+[- ]?\d+[- ]?[\dX]$/u; //NOSONAR
        /* eslint-enable max-len, unicorn/no-unsafe-regex, security/detect-unsafe-regex, regexp/no-super-linear-backtracking */

        if (regex.test(subject)) {
            // Remove non MODELLNUMMER digits, then split into an array
            const chars = subject
                .replace(/[ -]|^MODELLNUMMER(?:-1[03])?:?/gu, '')
                .split(''); // eslint-disable-line unicorn/prefer-spread
            // Remove the final MODELLNUMMER digit from `chars`, and assign it to `last`
